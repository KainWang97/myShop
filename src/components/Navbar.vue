...
const cartCount = computed(() => props.cartItems.reduce((acc, item) => acc + item.quantity, 0));
...
<template>
  <nav 
    ...
    :class="[
      isScrolled ? 'bg-washi/90 backdrop-blur-sm py-4 border-stone-200/100' : 'bg-transparent py-8 border-stone-200/0'
    ]"
  >
    <div class="max-w-7xl mx-auto px-6 flex justify-between items-center">
      ...
      <div class="flex items-center gap-8 text-sm tracking-widest text-stone-600 font-light">
        <!-- Admin Link -->
        <button 
          v-if="isAdmin"
        ...
        <button @click="emit('collection')" class="hover:text-sumi transition-colors hidden md:block">COLLECTION</button>
