...
<template>
  <div class="min-h-screen bg-washi pt-32 pb-24">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div v-if="filteredProducts.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-20 animate-fade-in">
        <ProductCard 
      ...
      <div v-else class="text-center py-20 text-stone-500 font-serif italic">
        {{ activeCategory === 'All' && products.length > 0 ? "All items are currently sold out." : "No items found in this category." }}
      </div>
    </div>
  </div>
</template>